...
<div class="worksheet-page">
  ...
  <section 
    class="dnd-zone" 
    use:dndzone={{items: $worksheetBlocks, flipDurationMs, dropTargetStyle: {outline: '2px dashed var(--accent)'}}} 
    on:consider={handleDndConsider} 
    on:finalize={handleDndFinalize}
  >
    {#each $worksheetBlocks as block, index (block.id)}
      <div class="problem-wrapper problem-block" animate:flip={{duration: flipDurationMs}}>
        
        <div class="block-controls problem-block-controls no-print">
          <span class="drag-handle">✋ Drag to move</span>
          <button class="remove-btn" on:click={() => removeBlock(block.id)}>✕</button>
        </div>
    ...
    {#if $worksheetBlocks.length === 0}
      <div class="empty-state no-print">
        ...
        <p>Drag or click problems from the sidebar to add them.</p>
      </div>
    {/if}
  </section>
